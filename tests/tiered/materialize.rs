...
fn test_materialize_roundtrip() {
    ...
    let (manifest, _vfs_name, bench) = import_and_open("mat_rt", cache_dir.path(), |conn| {
        ...
        for i in 0..500 {
            ...
            conn.execute(
                ...
                rusqlite::params![i, i % 50, format!("post body {} with some padding data", i)],
            )
            .unwrap();
        }
    });
    ...
    // Index scan
    assert_eq!(
        conn.query_row("SELECT COUNT(*) FROM posts WHERE user_id = 5", [], |r| r
            .get::<_, i64>(0))
            .unwrap(),
        10,
    );
...
fn test_materialize_single_page_db() {
    ...
    let conn = rusqlite::Connection::open(&output).unwrap();
    assert_eq!(
        conn.query_row("SELECT COUNT(*) FROM tiny", [], |r| r.get::<_, i64>(0))
            .unwrap(),
        1
    );
}
...
fn test_materialize_after_delete_and_vacuum() {
    ...
    let conn = rusqlite::Connection::open(&output).unwrap();
    assert_eq!(
        conn.query_row("SELECT COUNT(*) FROM big", [], |r| r.get::<_, i64>(0))
            .unwrap(),
        100
    );
}
...
fn test_materialize_large_page_size() {
    ...
    let local_db = cache_dir.path().join("source.db");
    {
        ...
        conn.execute_batch("PRAGMA page_size=65536; PRAGMA journal_mode=WAL;")
            .unwrap();
        conn.execute_batch("CREATE TABLE wide (id INTEGER PRIMARY KEY, data TEXT);")
            .unwrap();
        for i in 0..100 {
            conn.execute(
    ...
    assert_eq!(
        conn.query_row("SELECT COUNT(*) FROM wide", [], |r| r.get::<_, i64>(0))
            .unwrap(),
        100
    );
}
...
fn test_materialize_after_multiple_checkpoints() {
    ...
    let (_manifest, vfs_name, bench) = import_and_open("mat_multi", cache_dir.path(), |conn| {
        conn.execute_batch("CREATE TABLE log (id INTEGER PRIMARY KEY, msg TEXT);")
            .unwrap();
        for i in 0..50 {
            conn.execute(
    ...
    for round in 0..3 {
        ...
        for i in base..(base + 20) {
            conn.execute(
                "INSERT INTO log VALUES (?1, ?2)",
                rusqlite::params![i, format!("round{}_{}", round, i)],
            )
            .unwrap();
        }
        conn.execute_batch("PRAGMA wal_checkpoint(TRUNCATE);")
    ...
    assert!(
        version > 0,
        "version should be > 0 after multiple checkpoints, got {}",
        version
    );

    let conn = rusqlite::Connection::open(&output).unwrap();
    assert_eq!(
        conn.query_row("SELECT COUNT(*) FROM log", [], |r| r.get::<_, i64>(0))
            .unwrap(),
        110
    );
}
...
fn test_materialize_with_large_blobs() {
    ...
    let blob: Vec<u8> = conn
        .query_row("SELECT data FROM blobs WHERE id = 5", [], |r| r.get(0))
        .unwrap();
...
fn test_materialize_overwrites_existing_file() {
    let cache_dir = tempfile::tempdir().unwrap();
    let (_manifest, _vfs_name, bench) =
        import_and_open("mat_overwrite", cache_dir.path(), |conn| {
...
fn test_materialize_produces_writable_db() {
    ...
    for i in 100..150 {
        conn.execute(
            "INSERT INTO t VALUES (?1, ?2)",
            rusqlite::params![i, format!("added_{}", i)],
        )
        .unwrap();
    }
    ...
    conn.execute("UPDATE t SET val = 'modified' WHERE id = 0", [])
        .unwrap();
...
fn test_materialize_matches_source_bytes() {
    ...
    let local_db = cache_dir.path().join("source.db");
    {
        ...
        for i in 0..200 {
            conn.execute(
                ...
                rusqlite::params![i, format!("text_{}", i), i as f64 * 1.5],
            )
            .unwrap();
            conn.execute(
                "INSERT INTO t2 VALUES (?1, ?2, ?3)",
                rusqlite::params![i, i % 20, vec![i as u8; 100]],
            )
            .unwrap();
        }
        conn.execute_batch("PRAGMA wal_checkpoint(TRUNCATE);")
    ...
    assert_eq!(
        ...
        "file size mismatch: source={}, materialized={}",
        source_bytes.len(),
        materialized_bytes.len()
    );
    ...
    let first_diff = source_bytes
        .iter()
        .zip(materialized_bytes.iter())
        .enumerate()
        .find(|(_, (a, b))| a != b);

    if let Some((offset, (expected, actual))) = first_diff {
        panic!(
            "byte mismatch at offset {} (page {}): source=0x{:02x}, materialized=0x{:02x}",
            offset,
            offset / 4096,
            expected,
            actual
...
fn test_materialize_complex_queries() {
    ...
    let (_manifest, _vfs_name, bench) = import_and_open("mat_cplx", cache_dir.path(), |conn| {
        ...
        for i in 0..10 {
            conn.execute(
                ...
                rusqlite::params![i, format!("dept_{}", i)],
            )
            .unwrap();
        }
        for i in 0..1000 {
    ...
    assert_eq!(
        conn.query_row(
            "SELECT COUNT(DISTINCT d.name) FROM departments d JOIN employees e ON e.dept_id = d.id",
            [],
            |r| r.get::<_, i64>(0)
        )
        .unwrap(),
    ...
    // Aggregate
    let avg: f64 = conn
        .query_row("SELECT AVG(salary) FROM employees", [], |r| r.get(0))
        .unwrap();
