...
fn test_checkpoint_packs_new_pages_into_btree_groups() {
    ...
    let manifest = create_and_import(&config, &local_db, |conn| {
        ...
        for i in 0..200 {
            conn.execute(
                ...
                rusqlite::params![i, i % 50, format!("post body {}", i)],
            )
            .unwrap();
        }
    });
    ...
    // Cold reader: verify data from S3 (includes both import + VFS writes)
    {
        ...
        let idx_count: i64 = conn
            .query_row("SELECT COUNT(*) FROM posts WHERE user_id = 5", [], |row| {
                row.get(0)
            })
            .unwrap();
    ...
    let manifest = create_and_import(&config, &local_db, |conn| {
        ...
        for i in 0..500 {
            conn.execute(
                ...
                rusqlite::params![i, i % 20, format!("item data {} padding here", i)],
            )
            .unwrap();
        }
    });

    let total_groups = manifest
        .page_group_keys
        .iter()
        .filter(|k| !k.is_empty())
        .count();
...
fn test_vacuum_produces_correct_mapping() {
    ...
    create_and_import(&config, &local_db, |conn| {
        ...
        for i in 0..500 {
            conn.execute(
                "INSERT INTO vac_test VALUES (?1, ?2)",
                rusqlite::params![i, format!("row_{:04}_padding", i)],
            )
            .unwrap();
        }
    });
    ...
    let db_path = format!("file:vac.db?vfs={}", vfs_name);
    {
        ...
        .unwrap();
        conn.execute_batch("PRAGMA journal_mode=WAL;").unwrap();
        conn.execute("DELETE FROM vac_test WHERE id >= 200", [])
            .unwrap();
    ...
    for pn in 0..manifest.page_count {
        assert!(
            manifest.page_location(pn).is_some(),
            "after VACUUM, page {} should have a group assignment",
            pn
        );
    }
    ...
    // Cold reader should see 200 rows
    {
        ...
        .unwrap();
        let count: i64 = conn
            .query_row("SELECT COUNT(*) FROM vac_test", [], |row| row.get(0))
            .unwrap();
        assert_eq!(count, 200, "cold reader should see 200 rows after VACUUM");

        let idx_count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM vac_test WHERE data >= 'row_0100'",
                [],
                |row| row.get(0),
            )
            .unwrap();
        assert!(
            idx_count > 0 && idx_count <= 200,
            "index scan should work after VACUUM"
        );
    }
}
