...
fn staging_race_multi_table_interleave() {
    ...
    conn.execute_batch("CREATE TABLE IF NOT EXISTS orders (id INTEGER PRIMARY KEY, status TEXT);")
        .unwrap();
    conn.execute_batch("CREATE TABLE IF NOT EXISTS logs (id INTEGER PRIMARY KEY, msg TEXT);")
        .unwrap();

    // Checkpoint 1: data + orders
    insert_rows(&conn, 0, 50, "d1");
    conn.execute_batch("INSERT INTO orders VALUES (1, 'pending'), (2, 'pending');")
        .unwrap();
    conn.execute_batch("PRAGMA wal_checkpoint(TRUNCATE);")
        .unwrap();

    // Checkpoint 2: update orders, add logs (data untouched)
    conn.execute_batch("UPDATE orders SET status = 'shipped' WHERE id = 1;")
        .unwrap();
    conn.execute_batch("INSERT INTO logs VALUES (1, 'order 1 shipped');")
        .unwrap();
    conn.execute_batch("PRAGMA wal_checkpoint(TRUNCATE);")
        .unwrap();
    ...
    let s2: String = cold
        .query_row("SELECT status FROM orders WHERE id = 2", [], |r| r.get(0))
        .unwrap();
    assert_eq!(s2, "pending");
    // logs: 1 entry
    let log_count: i64 = cold
...
fn staging_scale_10k_rows() {
    ...
    // Spot check
    let v9999: String = cold
        .query_row("SELECT value FROM data WHERE id = 9999", [], |r| r.get(0))
        .unwrap();
...
fn staging_scale_many_small_checkpoints() {
    ...
    let v5: String = cold
        ...
        .unwrap();
    assert_eq!(v5, "b0_5");
    let v195: String = cold
        .query_row("SELECT value FROM data WHERE id = 195", [], |r| r.get(0))
        .unwrap();
...
fn staging_schema_create_index() {
    ...
    conn.execute_batch("CREATE INDEX idx_value ON data(value);")
        .unwrap();
    conn.execute_batch("PRAGMA wal_checkpoint(TRUNCATE);")
        .unwrap();
    ...
    assert_eq!(count, 500);
    let val: String = cold
        .query_row("SELECT value FROM data WHERE value = 'v1_250'", [], |r| {
            r.get(0)
        })
        .unwrap();
...
fn staging_schema_alter_table() {
    ...
    conn.execute_batch("ALTER TABLE data ADD COLUMN extra TEXT DEFAULT 'none';")
        .unwrap();
    conn.execute_batch("UPDATE data SET extra = 'filled_' || id WHERE id < 25;")
        .unwrap();
    conn.execute_batch("PRAGMA wal_checkpoint(TRUNCATE);")
        .unwrap();
    ...
    let extra10: String = cold
        .query_row("SELECT extra FROM data WHERE id = 10", [], |r| r.get(0))
        .unwrap();
    assert_eq!(extra10, "filled_10");
    let extra30: String = cold
        .query_row("SELECT extra FROM data WHERE id = 30", [], |r| r.get(0))
        .unwrap();
...
fn staging_crash_recover_then_continue() {
    ...
    assert!(
        ...
        "should recover staging from phase 1"
    );
...
fn staging_crash_two_checkpoints() {
    ...
    assert!(
        staging_log_count(cache_dir.path()) >= 2,
        "both staging logs should survive crash"
    );
    ...
    assert_eq!(v0, "cp1_0");
    let v99: String = conn
        .query_row("SELECT value FROM data WHERE id = 99", [], |r| r.get(0))
        .unwrap();
...
fn staging_crash_double() {
    ...
    assert_eq!(v0, "c1_0");
    let v59: String = conn
        .query_row("SELECT value FROM data WHERE id = 59", [], |r| r.get(0))
        .unwrap();
    assert_eq!(v59, "c2_59");
...
fn staging_active_reader_during_flush() {
    ...
    assert_eq!(
        count_after, 100,
        "reader should still see 100 rows after flush"
    );
...
fn staging_follower_no_staging_files() {
    ...
    assert_eq!(
        staging_log_count(follower_dir.path()),
        0,
        "read-only follower should create no staging files"
    );
}
...
fn staging_edge_readonly_no_staging() {
    ...
    assert_eq!(
        ...
        "no-op checkpoint should not create new staging log"
    );
}
...
fn staging_edge_large_single_txn() {
    ...
    let v4999: String = cold
        .query_row("SELECT value FROM data WHERE id = 4999", [], |r| r.get(0))
        .unwrap();
